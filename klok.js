...
async function checkPoints(apiClient, accountIndex) {
  try {
    ...
    log(`Tài khoản ${accountIndex + 1} | Points: ${pointsData.total_points || 0}`, 'custom');
...
async function runBot() {
  ...
  for (let i = 0; i < tokens.length; i++) {
    try {
      const account = await handleAccount(tokens[i], i);
  ...
  async function processAccounts() {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      try {
        const apiClient = createApiClient(account.token);
        ...
        const pointsBefore = await checkPoints(account.apiClient, account.accountIndex);
        ...
        const pointsAfter = await checkPoints(account.apiClient, account.accountIndex);
